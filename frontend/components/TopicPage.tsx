...
export function TopicPage({topic: initialTopic}:{topic:Topic}){
  ...
  return <div className="app-shell topic-shell"><Sidebar active="Live"/><main className="main"><Topbar compact/>
    ...
    <section className="panel drivers" data-slot="conversation-drivers" data-endpoint="GET /api/topics/{slug}/drivers"><h2>What’s driving the conversation? <Info size={13}/></h2><DataSlot name="driver-cards" endpoint="GET /api/topics/{slug}/drivers" hasData={topic.drivers.length>0}><div>{topic.drivers.map((driver,index)=>{const Icon=driverIcons[index%driverIcons.length];return <article key={driver.title}><i><Icon size={21}/></i><div><strong>{driver.title}</strong><p>{driver.description}</p><span className={driver.status.toLowerCase().replace(" ","-")}>{driver.status}</span></div></article>})}</div></DataSlot></section>
    <div className="topic-grid mid-grid">
      <section className="panel voices" data-slot="public-voices" data-endpoint="GET /api/topics/{slug}/voices"><h2>Voices from the public <Info size={13}/></h2><DataSlot name="voice-cards" endpoint="GET /api/topics/{slug}/voices" hasData={topic.voices.length>0} emptyTitle={topic.preview?"Public voices require collected comments":"Waiting for analysis"} emptyMessage={topic.preview?"Quotes are deliberately not fabricated for preview data. This panel will populate from anonymized, topic-matched comments.":undefined}><div>{topic.voices.map(voice=><article key={voice.label}><q>{voice.quote}</q><span className={voice.tone}>{voice.label}</span></article>)}</div></DataSlot><small>Representative, anonymized statements from the current analysis dataset.</small></section>
      <section className="panel confidence" data-slot="data-confidence" data-endpoint="GET /api/topics/{slug}/confidence"><h2>Data confidence <Info size={13}/></h2><DataSlot name="confidence-metrics" endpoint="GET /api/topics/{slug}/confidence" hasData={topic.confidence.sources.length>0}><div><article><Database/><span>Sources analysed<b>{topic.confidence.sources.join(", ")}</b></span></article><article><UsersRound/><span>{signalOnly?"Total public signals":"Total conversations"}<b>{topic.totalConversations.toLocaleString()}</b></span></article><article><Clock3/><span>Last updated<b>{topic.updated}</b></span></article><article><ShieldCheck/><span>AI confidence<b className="green">{topic.confidence.level}</b></span></article></div><p>{topic.confidence.qualified.toLocaleString()} qualified signals · {topic.confidence.lowSignal.toLocaleString()} low-signal items excluded or down-weighted</p></DataSlot></section>
    </div>
