...
def refresh_topic_analytics(db:Session,topic_slug:str)->None:
    ...
    analytics.setdefault("audience",{}).setdefault("language",{})["distribution"]=_distribution(languages,total);analytics["audience"].setdefault("geography",{})["value"]=geographies.most_common(1)[0][0] if geographies else "Not available from public source metadata";analytics["audience"]["geography"]["coverage"]=_percent(sum(geographies.values()),total);analytics["audience"]["age_bracket"]={"value":ages.most_common(1)[0][0] if ages else "Not available from public source metadata","confidence":"Observed" if ages else "Unavailable","coverage":_percent(sum(ages.values()),total)};analytics["audience"]["interest_groups"]=[name for name,_ in interests.most_common(3)] or ["No recurring interest theme detected"];analytics["audience"]["key_topics"]=[name for name,_ in interests.most_common(5)] or ["No recurring topic detected"];analytics["audience"]["leading_platform"]=f"{_human_platform(platforms.most_common(1)[0][0])} · {_percent(platforms.most_common(1)[0][1],total)}% of collected data"
